...
:: If program-level ALLOW rules were added before it, they would be deleted.
:: Adding them here (after PowerShell) ensures they survive and are active
:: when uvicorn starts — preventing the Windows security popup from appearing
:: and creating a new BLOCK rule.
netsh advfirewall firewall add rule name="PrintHub Venv Python Allow" dir=in action=allow program="!VENV_PYTHON!" enable=yes profile=any >nul 2>&1
if not "!SYS_PYTHON!"=="" (
...
echo [OK] Program-level ALLOW rules added for python.exe
echo.
